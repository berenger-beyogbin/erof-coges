  ...
  return <div className="space-y-5">
    ...
    {displayedRanking && [displayedRanking].map(({ drena, candidates }) => {
      ...
      return <section key={drena} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="p-4"><h4 className="mb-3 text-xs font-extrabold uppercase tracking-wide text-violet-700">1. Classement automatique combiné</h4>
        ...
        <div className="border-t border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center justify-between gap-3"><h4 className="text-xs font-extrabold uppercase tracking-wide text-emerald-700">2. Sélection définitive manuelle</h4><span className={`rounded-full px-2.5 py-1 text-[9px] font-bold uppercase ${session?.statut === 'valide' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{session ? session.statut : 'Non réalisée'}</span></div>
